...
| Caído vem de **Empurrar** (opção da ação de Ataque) ou de magia | Implementado: `ShoveAction` é a primeira fonte de condição dentro do motor. Magia continua fora | — |
| Empurrar usa **perícia**: Força (Atletismo) contra Força (Atletismo) ou Destreza (Acrobacia) | Teste de atributo cru, sem bônus de proficiência | Não existe perícia neste motor, e somá-la seria declarar todo mundo treinado. Medido, isso não é neutro: contra o arcanista a taxa fica **idêntica** (52,50%, porque os dois têm proficiência +2), e contra o duelista o brutamontes cai de 47,50% para 42,75%, porque a proficiência dele é +3. O campo não compra nada onde empata e distorce onde não empata |
| **O alvo escolhe** com que atributo resiste | Determinístico: o melhor de Força e Destreza, desempatando por Força | Não existe ponto no motor onde quem não é `turn_order.current` decida — `_validar_contexto` recusa com `NOT_YOUR_TURN`. "O melhor" é a única escolha que não precisa de justificativa, e o evento grava qual saiu |
| Empurrar tem **duas opções**: derrubar **ou** empurrar 5 pés | Só derrubar | Empurrar 5 pés precisa de uma direção e de uma casa livre, ou seja de um segundo modo de falha e de uma escolha dentro da ação. Entra quando houver quem a queira — e é ela que torna `KnockedProne` como evento separado indispensável, porque vencer a disputa passa a ter dois efeitos possíveis |
| O alvo deve ser no máximo **um tamanho maior** que quem empurra | Não há tamanho | `Statblock` não tem categoria de tamanho, e o único consumidor dela seria esta linha. Entra com a primeira criatura Grande |
| **Agarrar** (Grappling), o outro teste oposto da mesma seção da SRD | Não existe | Agarrado é condição com **duração e fonte** — ela termina quando o agarrador solta ou fica incapacitado —, e isso obriga `Condition` a virar dataclass. `rules.contest` já serve a ele; o que falta é o modelo de condição |
